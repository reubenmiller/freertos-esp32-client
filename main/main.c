...
void discover_tedge_broker(app_server_t *server, const char *service_name, const char *proto, const char *pattern)
{
    ...
    if (match != NULL)
    {
        ...
        // c8y proxy
        char c8y_proxy_host[256] = {0};
        sprintf(c8y_proxy_host, "http://%s.local:8001", match->hostname);
        server->c8y_proxy = strdup(c8y_proxy_host);
        ESP_LOGI(TAG, "Found matching service. mqtt=%s:%d, c8y_proxy=%s", server->mqtt_host, server->mqtt_port, server->c8y_proxy);
    }
    ...
    while (1)
    {
        ...
        else if (command.op_type == TEDGE_COMMAND_FIRMWARE_UPDATE)
        {
            ...
            case TEDGE_COMMAND_STATUS_EXECUTING:
                ...
                if (tedge_url != NULL)
                {
                    // transform remote url to c8y local proxy url
                    // TODO: remove once https://github.com/thin-edge/thin-edge.io/issues/2793 is implemented
                    // tedge_url = tedge_get_c8y_proxy_url(server.mqtt_host, remote_url);
